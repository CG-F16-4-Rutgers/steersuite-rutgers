...
namespace SteerLib
{
	...
	bool AStarPlanner::computePath(std::vector<Util::Point>& agent_path,  Util::Point start, Util::Point goal, SteerLib::SpatialDataBaseInterface * _gSpatialDatabase, bool append_to_path)
	{
		...
		//Create root of the node and add to openList//
		AStarPlannerNode* root = new AStarPlannerNode(start, double(0), double(0), double(0), NULL);
		openList.push_back(root);

		while (!openList.empty()) {
			int indexLeastF = minimumF(openList);
			AStarPlannerNode* mf = openList[indexLeastF];
			openList.erase(openList.begin() + indexLeastF);
			std::vector<AStarPlannerNode*> children = getNeighbors(n);
			for (int i = 0;i < children.size();i++) {
				if (children[i]->point == goal) {
					agent_path = pathFinder(successor[i]);
				}
			}
		}
		std::cout<<"\nIn A*";
