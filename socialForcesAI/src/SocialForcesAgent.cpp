...
//Additional definition Prefered Speed//
...
// Pursue and Evade//
Vector SocialForcesAgent::PursueAndEvade(Vector _goalDirection, float _dt)
{
	if (_color.r == 1.0f) {
	...
	for (std::vector<SteerLib::AgentInterface *>::iterator tmp_ag = neighbors.begin(); tmp_ag != neighbors.end(); tmp_ag++) {
		/*Blue agents pursue red agents*/
...
Util::Vector SocialForcesAgent::calcUnalignedCollisionAvoidance() {
	...
	std::vector<SteerLib::AgentInterface *> neighbor_agents = getNeighbors(search_radius);
	//std::cerr << neighbor_agents.size() << std::endl;
...
Util::Vector SocialForcesAgent::calcSeparationForce(float separation_radius, float separation_space) {
	std::vector<SteerLib::AgentInterface*> neighbor_agents = getNeighbors(separation_radius);
...
void SocialForcesAgent::updateAI(float timeStamp, float dt, unsigned int frameNumber)
{
	...
	//popuate neighbors//
	std::vector<SteerLib::AgentInterface*> neighbor_agents = getNeighbors(8.0f);
	...
	if (init_position.z >= 30.0f) {
		...
		std::vector<SteerLib::AgentInterface*> neighbor_agents = getNeighbors(4.0f);
